...
% Notes
% The data seems to follow a trend that we expect. The larger the data set the more sense it makes to update the VOID desciptions instead of generating them. We can show the graph of the data and the trend.
% From this we can talk about how is seems to be a good idea to update the VOID description instead of generating it, and that it is theorically feasible to do so.
% We can also show how accurate our data is with the bar error graph.
% We can show how the query size affects the time it takes to generate and update the VOID description.