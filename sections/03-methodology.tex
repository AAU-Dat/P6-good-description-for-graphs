...
An example of what the generate \gls{sparql} query could look like can be seen here:
"SELECT (COUNT(*) AS ?totalTriples) (COUNT(DISTINCT ?subject) AS ?numSubjects) (COUNT(DISTINCT ?predicate) AS ?numPredicates) (COUNT(DISTINCT ?object) AS ?numObjects) WHERE { ?subject ?predicate ?object . }"
...
An example of what the update \gls{sparql} query could look like can be simplified to four different queries combined using UNION. The first part of the query would look at the subjects: "SELECT ?resource (EXISTS { ?resource ?p ?o } AS ?existing) { VALUES ?resource { \textbf{Example\_of\_subject} } }"

The second part of the query would look for predicates: "SELECT ?resource (EXISTS { ?s ?resource ?o } AS ?existing) { VALUES ?resource { \textbf{Example\_of\_predicate} } }"

The third part of the query would look for objects: "SELECT ?resource (EXISTS { ?s ?p ?resource } AS ?existing) { VALUES ?resource { \textbf{Example\_of\_object} } }"

The fourth and final part of the query that would look for triples: "SELECT ?triple ?existing { VALUES (?s ?p ?o) {  (\textbf{Subject   Predicate   Object} )(\textbf{Subject    Predicate   Object} )} BIND(CONCAT(str(?s), str(?p), str(?o)) AS ?triple) BIND(EXISTS { ?s ?p ?o } AS ?existing) }"

The four queries would be combined using UNION, and the results would be used to update the \gls{void} description. The first three parts of the query search the dataset to see if a given subject, predicate, or object exists, and the fourth part searches for a given triple in the dataset. This is done to create new values for the \gls{void} description or to update existing values.
